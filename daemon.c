...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (mutex01) {
		mutex01--;
		printf("m01: LOCK daemon->per_ip_connection_mutex\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (mutex02) {
		mutex02--;
		printf("m02: UNLOCK daemon->per_ip_connection_mutex\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug7) {
		debug7--;
		printf("d07: connection.MHD_connection_handle_idle() as connection->idle_handler() will be called POINT 1 from daemon.call_handlers()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug8) {
		debug8--;
		printf("d08: connection.MHD_connection_handle_idle() as connection->idle_handler() will be called POINT 2 from daemon.call_handlers()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug12) {
		debug12--;
		printf("d12: daemon.call_handlers() will be called POINT 1 from daemon.MHD_handle_connection()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug13) {
		debug13--;
		printf("d13: daemon.call_handlers() will be called POINT 2 from daemon.MHD_handle_connection()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug9) {
		debug9--;
		printf("d09: connection.MHD_connection_handle_idle() as connection->idle_handler() will be called from daemon.MHD_handle_connection()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug21) {
		debug21--;
		printf("d21: daemon.MHD_select_thread() as start_routine() will be called from daemon.create_thread()\n");
	}
...
uint nextWorker = 0;	//*************************** MODIFIED (added) SINGLE STRING HERE!!! ****************************
...
//	worker = &daemon->worker_pool[(i + client_socket) % daemon->worker_pool_size];
	...
	worker = &daemon->worker_pool[nextWorker];
	nextWorker++;

	if (nextWorker >= daemon->worker_pool_size) {
		nextWorker = 0;
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug27) {
		debug27--;
		printf("d27: connection sended to thread in daemon.internal_add_connection()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (mutex03) {
		mutex03--;
		printf("m03: LOCK daemon->cleanup_connection_mutex\n");
	}
...
if  ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex04) {
		mutex04--;
		printf("m04: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (mutex05) {
		mutex05--;
		printf("m05: LOCK daemon->cleanup_connection_mutex\n");
	}
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex06) {
		mutex06--;
		printf("m06: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (mutex07) {
		mutex07--;
		printf("m07: LOCK daemon->cleanup_connection_mutex\n");
	}
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex08) {
		mutex08--;
		printf("m08: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************	
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex09) {
		mutex09--;
		printf("m09: LOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************	
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex10) {
		mutex10--;
		printf("m10: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************		
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex11) {
		mutex11--;
		printf("m11: LOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************	
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex12) {
		mutex12--;
		printf("m12: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************
...
//*************************** MODIFIED HERE!!! ****************************	
	...
	master = MHD_get_master(daemon);
  
	if (MHD_YES != MHD_mutex_lock_ (&master->accept_mutex)) {
		MHD_PANIC ("Failed to acquire accept mutex\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug28) {
		debug28--;
		printf("d28: unblocked ACCEPTing connection in daemon.MHD_accept_connection()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************	
	...
	if (MHD_YES != MHD_mutex_unlock_ (&master->accept_mutex)) {
		MHD_PANIC ("Failed to release accept mutex\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug22) {
		debug22--;
		printf("d22: ACCEPT connection in daemon.MHD_accept_connection()\n");
	}
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex13) {
		mutex13--;
		printf("m13: LOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex14) {
		mutex14--;
		printf("m14: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug23) {
		debug23--;
		printf("d23: daemon.MHD_accept_connection() will be called from daemon.MHD_run_from_select()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug14) {
		debug14--;
		printf("d14: daemon.call_handlers() will be called from daemon.MHD_run_from_select()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug15) {
		debug15--;
		printf("d15: daemon.call_handlers() will be called from daemon.MHD_poll_all()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug24) {
		debug24--;
		printf("d24: daemon.MHD_accept_connection() will be called from daemon.MHD_poll_all()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug25) {
		debug25--;
		printf("d25: daemon.MHD_accept_connection() will be called from daemon.MHD_poll_listen_socket()\n");
	}
...
#define MAX_EVENTS 256		//*************************** MODIFIED SINGLE STRING HERE!!! ****************************
#define ACCEPTS_COUNT 1		//*************************** MODIFIED (added) SINGLE STRING HERE!!! ****************************
 ...
 */
static int
MHD_epoll (struct MHD_Daemon *daemon, int may_block) {
	struct MHD_Connection *pos;
	struct MHD_Connection *next;
	...
	if (-1 == daemon->epoll_fd) {	/* we're down! */
		return MHD_NO; 
	}
...
//*************************** MODIFIED HERE!!! ****************************
				...
				if (debug26) {
					debug26--;
					printf("d26: daemon.MHD_accept_connection() will be called from daemon.MHD_epoll()\n");
				}
...
//*************************** MODIFIED END ********************************
				...
				while ( (MHD_YES == MHD_accept_connection (daemon)) &&
						(daemon->connections < daemon->connection_limit) &&
						(series_length < ACCEPTS_COUNT) )							//*************************** MODIFIED SINGLE STRING HERE!!! ****************************
//						(series_length < 128) )
				{
					series_length++;
				}
...
//*************************** MODIFIED HERE!!! ****************************
			...
			if (debug16) {
				debug16--;
				printf("d16: daemon.call_handlers() will be called from daemon.MHD_epoll()\n");
			}
...
//*************************** MODIFIED HERE!!! ****************************
		...
		if (debug10) {
			debug10--;
			printf("d10: connection.MHD_connection_handle_idle() as connection->idle_handler() will be called POINT 1 from daemon.MHD_epoll()\n");
		}
...
//*************************** MODIFIED HERE!!! ****************************
		...
		if (debug11) {
			debug11--;
			printf("d11: connection.MHD_connection_handle_idle() as connection->idle_handler() will be called POINT 2 from daemon.MHD_epoll()\n");
		}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug17) {
		debug17--;
		printf("d17: daemon.MHD_epoll() will be called from daemon.MHD_run()\n");
	}
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug18) {
		debug18--;
		printf("d18: daemon.MHD_epoll() will be called from daemon.MHD_select_thread()\n");
	}
...
if (MHD_YES != MHD_mutex_create_ (&daemon->accept_mutex))
    {
#ifdef HAVE_MESSAGES
      MHD_DLOG (daemon,
		"MHD failed to initialize accept_mutex\n");
#endif
#if HTTPS_SUPPORT
      if (0 != (flags & MHD_USE_SSL))
	gnutls_priority_deinit (daemon->priority_cache);
       ...
       (0 == (daemon->options & MHD_USE_NO_LISTEN_SOCKET)))
{
	if (debug19) {
		debug19--;
		printf("d19: daemon.MHD_select_thread() in create_thread() will be called POINT 1 from daemon.MHD_start_daemon_va()\n");
	}
}	
...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug20) {
		debug20--;
		printf("d20: daemon.MHD_select_thread() in create_thread() will be called from daemon.MHD_start_daemon_va()\n");
	}
...
thread_failed:
 ...
 free_and_fail:
  ...
  (void) MHD_mutex_destroy_ (&daemon->accept_mutex);	//*************************** MODIFIED (added) SINGLE STRING HERE!!! ****************************
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex15) {
		mutex15--;
		printf("m15: LOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************
...
if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))) {
	if (mutex16) {
		mutex16--;
		printf("m16: UNLOCK daemon->cleanup_connection_mutex\n");
	}
}
//*************************** MODIFIED END ********************************
