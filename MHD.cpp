...
//*************************** MODIFIED HERE!!! ****************************
	...
	if (debug0) {
		debug0--;
		printf("d00: node.run() will be called from mhd.mhd_request_callback()\n\n");
	}

//*************************** MODIFIED END ********************************	

//        mhd_server->run(MHD_get_thread_number(connection), *req, resp);
		mhd_server->run(MHD_get_connection_info(connection, MHD_CONNECTION_INFO_CLIENT_ADDRESS), MHD_get_thread_number(connection), *req, resp);		//*************************** MODIFIED SINGLE STRING HERE!!! ****************************
    }
...
bool MHD::start(const string& path)
{
    ...
    if (host.empty()) {
        d = MHD_start_daemon(MHD_USE_EPOLL_INTERNALLY_LINUX_ONLY | MHD_USE_EPOLL_TURBO, port, nullptr,
                             nullptr, &mhd_request_callback, this, 
